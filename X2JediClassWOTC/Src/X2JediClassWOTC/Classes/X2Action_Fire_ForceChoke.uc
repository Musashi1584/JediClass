...
function Init()
{
	...
	foreach FoundFireWeaponNotifyEvents(NotifyEvent)
	{
		...
		`LOG(default.class @ GetFuncName() @ `ShowVar(NotifyEvent.Time) @ `ShowVar(Notify),, 'X2JediClassWOTC');
		break;
	}
...
{	
	`LOG(default.class @ GetFuncName() @ `ShowVar(bMainImpactNotify) @ `ShowVar(HitLocation),, 'X2JediClassWOTC');
}
