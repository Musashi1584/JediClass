...
event Notify(Actor Owner, AnimNodeSequence AnimSeqInstigator)
{
	...
	`LOG(default.Class @ GetFuncName() @ Owner @ Pawn.ObjectID,, 'X2JediClassWotc');
	...
	//
	//	`LOG(default.Class @ GetFuncName() @ UnitState @ PrimaryWeapon @ SecondaryWeapon,, 'X2JediClassWotc');
	//
	...
	for (i = 0; i < UnitSkeletalMesh.Attachments.Length; ++i)
	{
		if (UnitSkeletalMesh.Attachments[i].Component != none)
		{
			...
			if (Weapon != none)
			{
				`LOG(default.Class @ GetFuncName() @ Weapon.ObjectArchetype,, 'X2JediClassWotc');
