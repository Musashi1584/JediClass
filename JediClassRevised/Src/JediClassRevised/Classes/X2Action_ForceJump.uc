...
function Init()
{
	...
	`LOG(default.class @ GetFuncName() @ `ShowVar(AbilityContext) @ `ShowVar(DesiredLocation),, 'JediClassRevised');
}
...
function NotifyEnvironmentDamage(int PreviousPathTileIndex, bool bFragileOnly = true, bool bCheckForDestructibleObject = false)
{
	...
	for(Index = PreviousPathTileIndex; Index <= PathTileIndex; ++Index)
	{
		...
		{
			`log(default.class @ GetFuncName() @ `showvar(EnvironmentDamage),, 'JediClassRevised');
			...
			`log(default.class @ GetFuncName() @ `showvar(TileLocation),, 'JediClassRevised');
			`log(default.class @ GetFuncName() @ `showvar(HitLocation),, 'JediClassRevised');
			`log(default.class @ GetFuncName() @ `showvar(DestroyTileDistance),, 'JediClassRevised');
			...
			{				
				...
				`log(default.class @ GetFuncName() @ "Shots fired!!",, 'JediClassRevised');
			}
		}
	}
}
...
simulated function bool MoveAlongPath(float fTime, XComUnitPawn pActor)
{
	...
	//`LOG(GetFuncName() @ `ShowVar(TargetLocation.Z) @ `ShowVar(KF.vLoc.Z),, 'JediClassRevised');
...
function InterpolatePath(float StartZ)
{
	...
	for(Index = 0; Index < Path.iNumKeyframes; Index++)
	{
		...
		//`LOG(GetFuncName() @ `ShowVar(Diff),, 'JediClassRevised');
...
Begin:
	...
	`LOG(default.class @ GetFuncName() @ `ShowVar(Params.AnimName),, 'JediClassRevised');
	...
	`LOG(default.class @ GetFuncName() @ `ShowVar(UnitPawn.Location),, 'JediClassRevised');
