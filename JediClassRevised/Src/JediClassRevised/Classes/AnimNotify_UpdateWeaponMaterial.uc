...
event Notify(Actor Owner, AnimNodeSequence AnimSeqInstigator)
{
	...
	`LOG(default.Class @ GetFuncName() @ Owner @ Pawn.ObjectID,, 'JediClassRevised');
	...
	//
	//	`LOG(default.Class @ GetFuncName() @ UnitState @ PrimaryWeapon @ SecondaryWeapon,, 'JediClassRevised');
	//
	...
	for (i = 0; i < UnitSkeletalMesh.Attachments.Length; ++i)
	{
		if (UnitSkeletalMesh.Attachments[i].Component != none)
		{
			...
			if (Weapon != none)
			{
				`LOG(default.Class @ GetFuncName() @ Weapon.ObjectArchetype,, 'JediClassRevised');
