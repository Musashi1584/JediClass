...
function Init()
{
	...
	`LOG(default.class @ AbilityContext.ResultContext.HitResult @ bHit,, 'JediClassRevised');
...
function SendProjectile(AnimNotify_FireWeaponVolley FireVolleyNotify)
{
	...
	//`log("X2Action_LightsaberDeflect DeflectTarget" @ SourceUnitState.ObjectID @ ReflectTargetID,, 'JediClassRevised');
	...
	//`log("X2Action_LightsaberDeflect Target Unit" @ TargetUnitState.GetMyTemplateName() @ ReflectTargetID @ Target,, 'JediClassRevised');
	if (bHit)
	{
	...
	else
	{
		...
		`log("X2Action_LightsaberDeflect Randomize Target" @ `ShowVar(Target),, 'JediClassRevised');
		SpawnAndConfigureNewCosmeticProjectile(ProjectileTemplate, FireVolleyNotify, AbilityContext, Source, Target);
	}

	`log("X2Action_LightsaberDeflect SendProjectile" @ bHit @ ProjectileTemplate.ObjectArchetype,, 'JediClassRevised');
}
...
function EventListenerReturn OnProjectileFromActionToWaitFor(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	if (!bSendProjectiles)
	{
		`log(default.Class @ GetFuncName() @ "sending projectiles",, 'JediClassRevised');
...
{
	`log(default.Class @ GetFuncName() @ "stopping anim",, 'JediClassRevised');
...
function CompleteAction()
{
	...
	`LOG("X2Action_LightsaberDeflect.CompleteAction",, 'JediClassRevised');

	if (AbilityContext.ResultContext.EffectRedirects.Length > 0)
	{
		`LOG("X2Action_LightsaberDeflect Reseting bCanDamageNonFragile for " @ AbilityContext.InputContext.SourceObject.ObjectID,, 'JediClassRevised');
...
simulated state Executing
{
	simulated event Tick(float fDeltaT)
	{
		...
		//`log(default.Class @ GetFuncName() @ "bSendProjectiles" @ bSendProjectiles,, 'JediClassRevised');
		if (bSendProjectiles && !bProjectilesFired)
		{
			if (FireVolleyNotifies.Length == 0)
			{
				...
				`log("X2Action_LightsaberDeflect FireVolleyNotify" @ InstigatorUnitState.GetMyTemplateName() @ InstigatorUnitState @ FireVolleyNotifies.Length @ ProjectileTemplate,, 'JediClassRevised');
			}
			...
			if (OutNotifyTimes.Length > 0 && CurrentVolleyIndex < OutNotifyTimes.Length)
			{
				...
				{
					`log("X2Action_LightsaberDeflect Sending Volley" @ CurrentVolleyIndex @ NextProjectileTime @ ProjectileTimer,, 'JediClassRevised');
			...
			{
				`log("X2Action_LightsaberDeflect End anim",, 'JediClassRevised');
				bProjectilesFired = true;
			}
			else if (DummyNotify != none)
			{
				`log("X2Action_LightsaberDeflect Send dummy projectile",, 'JediClassRevised');
...
Begin:
	...
	if (UnitPawn.GetAnimTreeController().CanPlayAnimation(Params.AnimName))
	{
		...
		else
		{
			...
			//	FinishAnim(PlayingSequence);
			//}
			`log("X2Action_LightsaberDeflect FinishAnim",, 'JediClassRevised');
