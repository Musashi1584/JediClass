...
static simulated function PrecomputedPathMovement_ModifyActivatedAbilityContext(XComGameStateContext Context)
{
	...
	if (EndTileLocation != EmptyTile)
	{
		...
		`LOG(GetFuncName() @ EndTileLocation.X @ EndTileLocation.Y @ EndTileLocation.Z,, 'JediClassRevised');
	}
	...
	if (AbilityContext != none && AbilityContext.InterruptionStatus != eInterruptionStatus_Interrupt)
	{
		foreach AbilityContext.InputContext.MultiTargets(TargetRef)
		{
			...
			if (TargetUnit != none)
			{
				...
				if (DamageDealt > 0)
				{
					//`LOG("TargetUnit" @ TargetUnit.GetMyTemplateName() @ DamageDealt,, 'JediClassRevised');
		...
		if (StolenHP > 0)
		{
			...
			if (NewSourceUnit != none)
			{
				...
				//`LOG("ForceDrainListener AbilityTriggerAgainstSingleTarget" @ AbilityState.ObjectID,, 'JediClassRevised');
...
simulated function ForceChoke_BuildVisualization(XComGameState VisualizeGameState/*, out array<VisualizationTrack> OutVisualizationTracks*/)
{
	...
	if(Context.IsResultContextHit())
	{
		...
		//`LOG(AbilityTemplate.AbilityTargetEffects[0].Name @ Context.FindTargetEffectApplyResult(AbilityTemplate.AbilityTargetEffects[0]),, 'JediClassRevised');
