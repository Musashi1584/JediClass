...
// if set to true, missed units will be used for visualization purposes
var config bool bAlsoHitMissedUnits;
...
function BuildTargetPath()
{
	...
	{
		if (Targets[i].isHit || bAlsoHitMissedUnits)
		{
			OpenSet.AddItem(i);
			hitTargets++;
		}
...
{
	`LOG("ProjectileNotifyHit",, 'JediClassRevised');
...
function OnProjectileHit(vector vLocation)
{
	...
	`LOG("OnProjectileHit",, 'JediClassRevised');
	if (!ChainHasStarted())
	{
	...
	else
	{
		...
		`LOG("FindNearestTarget" @ i,, 'JediClassRevised');
	}
	if (i != INDEX_NONE)
	{
...
function AdvanceChain(int iHitIdx, bool bDoNotifyTarget, vector vSourceLocation)
{
	...
	if (bDoNotifyTarget)
	{
		...
		`LOG(default.Class @ GetFuncName() @ "sent Visualizer_ProjectileHit to" @ TargetState,, 'JediClassRevised');
	}
	for (i = 1; i < Targets.Length; i++)
	{
...
function SendProjectile(vector Source, vector Target)
{
	...
	local X2UnifiedProjectile NewProjectile, ProjectileTemplate;
	...
	`LOG("ProjectileTemplate" @ ProjectileTemplate,, 'JediClassRevised');
...
Begin:
	...
	`LOG("Perks for" @ AbilityContext.InputContext.AbilityTemplateName @ Perks.Length,, 'JediClassRevised');
	`LOG("Unit.CurrentPerkAction" @ Unit.CurrentPerkAction,, 'JediClassRevised');
