...
function Init()
{
	...
	`LOG(default.class @ GetFuncName() @ `ShowVar(AbilityContext) @ `ShowVar(DesiredLocation),, 'JediClassWOTCV2');
}
...
function NotifyEnvironmentDamage(int PreviousPathTileIndex, bool bFragileOnly = true, bool bCheckForDestructibleObject = false)
{
	...
	for(Index = PreviousPathTileIndex; Index <= PathTileIndex; ++Index)
	{
		...
		{
			`log(default.class @ GetFuncName() @ `showvar(EnvironmentDamage),, 'JediClassWOTCV2');
			...
			`log(default.class @ GetFuncName() @ `showvar(TileLocation),, 'JediClassWOTCV2');
			`log(default.class @ GetFuncName() @ `showvar(HitLocation),, 'JediClassWOTCV2');
			`log(default.class @ GetFuncName() @ `showvar(DestroyTileDistance),, 'JediClassWOTCV2');
			...
			{				
				...
				`log(default.class @ GetFuncName() @ "Shots fired!!",, 'JediClassWOTCV2');
			}
		}
	}
}
...
simulated function bool MoveAlongPath(float fTime, XComUnitPawn pActor)
{
	...
	//`LOG(GetFuncName() @ `ShowVar(TargetLocation.Z) @ `ShowVar(KF.vLoc.Z),, 'JediClassWOTCV2');
...
function InterpolatePath(float StartZ)
{
	...
	for(Index = 0; Index < Path.iNumKeyframes; Index++)
	{
		...
		//`LOG(GetFuncName() @ `ShowVar(Diff),, 'JediClassWOTCV2');
...
Begin:
	...
	`LOG(default.class @ GetFuncName() @ `ShowVar(Params.AnimName),, 'JediClassWOTCV2');
	...
	`LOG(default.class @ GetFuncName() @ `ShowVar(UnitPawn.Location),, 'JediClassWOTCV2');
