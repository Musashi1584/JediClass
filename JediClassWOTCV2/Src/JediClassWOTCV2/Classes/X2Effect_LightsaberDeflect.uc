...
)
{
	...
	foreach default.IgnoreEffects(CurrentEffect)
	{
		...
		{
			`LOG(default.class @ GetFuncName() @ "false, IgnoreEffects" @ CurrentEffect,, ,, 'JediClassWOTCV2');
			return false;
		}
	}
	...
	{
		`LOG(default.class @ GetFuncName() @ "false, is in overwatch",, ,, 'JediClassWOTCV2');
		return false;
	}
	...
	if (!AbilityState.IsMeleeAbility() && bIsPrimaryTarget)
	{
		...
		if (RandRoll < ModifiedHitChance)
		{
			...
			`LOG(default.class @ GetFuncName() @ NewHitResult @ bReflect,, 'JediClassWOTCV2');
			return true;
		}
	}
