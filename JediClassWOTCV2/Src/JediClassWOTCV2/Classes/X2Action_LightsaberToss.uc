...
{
	`log(default.Class @ GetFuncName() @ "AddProjectileVolley",, 'JediClassWOTCV2');
...
function Init()
{
	...
	`log(default.Class @ GetFuncName() @ LastLocation.X @ LastLocation.Y @ LastLocation.Z,, 'JediClassWOTCV2');
...
function SetProjectileColor()
{
	...
	if (MeshComp != none)
	{
		for (i = 0; i < MeshComp.GetNumElements(); i++)
		{
			...
			//`log(default.class @ GetFuncName() @ "MIC" @ i @ MIC @ MIC.Parent,, 'JediClassWOTCV2');
			if (InStr(MIC.Parent, "MAT_Lightsaber_Blade") != INDEX_NONE)
			{
				//`log(default.class @ GetFuncName() @ "MIC" @ "applying",, 'JediClassWOTCV2');
			...
			//`log(default.class @ GetFuncName() @ "MITV" @ i @ MITV @ MITV.Parent,, 'JediClassWOTCV2');
			if (InStr(MITV.Parent, "MAT_Lightsaber_Blade") != INDEX_NONE)
			{
				//`log(default.class @ GetFuncName() @ "MITV" @ "applying",, 'JediClassWOTCV2');
...
function SendProjectile(vector Source, vector Target, bool bReturnToSource = false)
{
	...
	`log("X2Action_LightsaberToss SendProjectile to" @ Target,, 'JediClassWOTCV2');
}

function NotifyTargetsAbilityApplied()
{
	`log("X2Action_LightsaberToss" @ GetFuncName() @ "was called",, 'JediClassWOTCV2');
}
...
function ProjectileNotifyHit(bool bMainImpactNotify, Vector HitLocation)
{
	...
	`log("X2Action_LightsaberToss ProjectileNotifyHit" @ bMainImpactNotify @ iProjectileNotifyHitIndex @ "/" @ Targets.Length @ HitLocation @ UnitPawn.Location,, 'JediClassWOTCV2');
	...
	if (bReturn)
	{
		...
		`log("X2Action_LightsaberToss bMainImpactNotify" @ bMainImpactNotify @ "/" @ iProjectileNotifyHitIndex @ "/" @ Targets.Length,, 'JediClassWOTCV2');
		`log("X2Action_LightsaberToss HitLocation" @ HitLocation,, 'JediClassWOTCV2');
		`log("X2Action_LightsaberToss RightHandLocation" @ RightHandLocation,, 'JediClassWOTCV2');
		...
		for (HandProximity = 0.0; HandProximity <= 100.0;  HandProximity+=0.1)
		{
			...
			{
				`log("X2Action_LightsaberToss AreVectorsDifferent" @ HandProximity @ class'Helpers'.static.AreVectorsDifferent(HitLocation, RightHandLocation, HandProximity),, 'JediClassWOTCV2');
				`log("X2Action_LightsaberToss returned to source" @ iProjectileNotifyHitIndex @ Targets.Length,, 'JediClassWOTCV2');
...
simulated state Executing
{
	...
	{
		//`log(bSnap,, 'JediClassWOTCV2');
		if (bSnap && !bCanComplete)
		{
			//`log("iNextNotifyIndex" @ iNextNotifyIndex @ "Targets.Length" @ Targets.Length,, 'JediClassWOTCV2');
			DoSnap();
	...
	simulated function DoSnap()
	{
		...
		`log(default.Class @ GetFuncName() @ iNextNotifyIndex @Targets.Length,, 'JediClassWOTCV2');

		if (iNextNotifyIndex >= Targets.Length)
		{
			UnitPawn.Mesh.GetSocketWorldLocationAndRotation('R_Hand', NextTargetLocation);
			`log("X2Action_LightsaberToss Return Lightsaber to location" @ NextTargetLocation,, 'JediClassWOTCV2');
		...
		else if (iNextNotifyIndex < Targets.Length)
		{
			...
			{
				`log("X2Action_LightsaberToss Shooting Lightsaber at target" @ NextTargetLocation @ Targets.Length @ iNextNotifyIndex,, 'JediClassWOTCV2');
...
Begin:
	...
	if (!bCanComplete)
	{
		`LOG("Start" @ IdleAnimationName,, 'JediClassWOTCV2');
	...
	if (bCanComplete && bReturnedToSource)
	{
		...
		`LOG(default.class @ "Start" @ StopAnimationName,, 'JediClassWOTCV2');
		LightsaberReturned();
		Params = default.Params;
