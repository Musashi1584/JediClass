...
static function CHEventListenerTemplate CreateListenerTemplate_OnSquaddieItemStateApplied()
{
	...
	`log("Register Event SquaddieItemStateApplied",, 'JediClassWOTCV2');
...
static function CHEventListenerTemplate CreateListenerTemplate_OnOverrideUnitFocusUI()
{
	...
	`log("Register Event OverrideUnitFocusUI",, 'JediClassWOTCV2');
...
static function CHEventListenerTemplate CreateListenerTemplate_OnSoldierInfo()
{
	...
	`LOG("Register Event SoldierClassIcon",, 'JediClassWOTCV2');

	Template.AddCHEvent('SoldierClassDisplayName', OnSoldierInfo, ELD_Immediate);
	`LOG("Register Event SoldierClassDisplayName",, 'JediClassWOTCV2');

	Template.AddCHEvent('SoldierClassSummary', OnSoldierInfo, ELD_Immediate);
	`LOG("Register Event SoldierClassSummary",, 'JediClassWOTCV2');
...
static function EventListenerReturn OnSoldierInfo(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	...
	//`LOG(GetFuncName() @ UnitState.GetFullName(),, 'JediClassWOTCV2');

	if (UnitState.GetSoldierClassTemplate().DataName != 'Jedi')
	{
		//`LOG(GetFuncName() @ "bailing" @ UnitState.GetSoldierClassTemplate().DisplayName @ UnitState.GetSoldierClassTemplate().DataName,, 'JediClassWOTCV2');
		return ELR_NoInterrupt;
	}
	...
	switch (Event)
	{
		...
		case 'SoldierClassDisplayName':
			//`LOG(GetFuncName() @ Event @ class'JediClassHelper'.static.GetDarkSideModifier(UnitState),, 'JediClassWOTCV2');
	...
	//`LOG(GetFuncName() @ Event @ Info,, 'JediClassWOTCV2');
