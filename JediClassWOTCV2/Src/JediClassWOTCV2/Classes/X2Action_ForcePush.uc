...
simulated state Executing
{
	...
	function MaybeNotifyEnvironmentDamage()
	{
		...
		//`LOG("X2Action_ForcePush MaybeNotifyEnvironmentDamage" @ CurrentTile.X @ CurrentTile.Y @ CurrentTile.Z,, 'JediClassWOTCV2');

		foreach StateChangeContext.AssociatedState.IterateByClassType(class'XComGameState_EnvironmentDamage', EnvironmentDamage)
		{
			//`LOG("X2Action_ForcePush EnvironmentDamage" @ EnvironmentDamage.HitLocationTile.X @ EnvironmentDamage.HitLocationTile.Y @ EnvironmentDamage.HitLocationTile.Z,, 'JediClassWOTCV2');
			if(EnvironmentDamage.HitLocationTile.X == CurrentTile.X && EnvironmentDamage.HitLocationTile.Y == CurrentTile.Y)
			{
				...
				//`LOG("X2Action_ForcePush Notify Environmentdamage" @ DmgObjectRef.ObjectID,, 'JediClassWOTCV2');
			}
		}
	}
...
Begin:
	...
	{
		//`LOG("X2Action_ForcePush AddImpulse" @ DistanceToTargetSquared,, 'JediClassWOTCV2');
	...
	While (DistanceToTargetSquared > CloseEnoughDistance && iTries < 50)
	{
		...
		sleep (0.0f);
		DistanceToTargetSquared = VSizeSq2D(EndingLocation - UnitPawn.Location);
		//`LOG("X2Action_ForcePush" @ DistanceToTargetSquared @ iTries,, 'JediClassWOTCV2');
	}
