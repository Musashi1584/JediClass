...
event Notify(Actor Owner, AnimNodeSequence AnimSeqInstigator)
{
	...
	`LOG(default.Class @ GetFuncName() @ Owner @ Pawn.ObjectID,, 'JediClassWOTCV2');
	...
	//
	//	`LOG(default.Class @ GetFuncName() @ UnitState @ PrimaryWeapon @ SecondaryWeapon,, 'JediClassWOTCV2');
	//
	...
	for (i = 0; i < UnitSkeletalMesh.Attachments.Length; ++i)
	{
		if (UnitSkeletalMesh.Attachments[i].Component != none)
		{
			...
			if (Weapon != none)
			{
				`LOG(default.Class @ GetFuncName() @ Weapon.ObjectArchetype,, 'JediClassWOTCV2');
