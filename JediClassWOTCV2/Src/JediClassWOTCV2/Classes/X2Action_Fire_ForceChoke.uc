...
function Init()
{
	...
	foreach FoundFireWeaponNotifyEvents(NotifyEvent)
	{
		...
		`LOG(default.class @ GetFuncName() @ `ShowVar(NotifyEvent.Time) @ `ShowVar(Notify),, 'JediClassWOTCV2');
		break;
	}
...
{	
	`LOG(default.class @ GetFuncName() @ `ShowVar(bMainImpactNotify) @ `ShowVar(HitLocation),, 'JediClassWOTCV2');
}
