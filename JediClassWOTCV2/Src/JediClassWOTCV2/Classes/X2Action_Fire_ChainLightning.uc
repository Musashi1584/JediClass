...
{
	`LOG("ProjectileNotifyHit",, 'JediClassWOTCV2');
...
function OnProjectileHit(vector vLocation)
{
	...
	`LOG("OnProjectileHit",, 'JediClassWOTCV2');
	if (!ChainHasStarted())
	{
	...
	else
	{
		...
		`LOG("FindNearestTarget" @ i,, 'JediClassWOTCV2');
	}
	if (i != INDEX_NONE)
	{
...
function AdvanceChain(int iHitIdx, bool bDoNotifyTarget, vector vSourceLocation)
{
	...
	if (bDoNotifyTarget)
	{
		...
		`LOG(default.Class @ GetFuncName() @ "sent Visualizer_ProjectileHit to" @ TargetState,, 'JediClassWOTCV2');
	}
	for (i = 1; i < Targets.Length; i++)
	{
...
function SendProjectile(vector Source, vector Target)
{
	...
	`LOG("ProjectileTemplate" @ ProjectileTemplate,, 'JediClassWOTCV2');
...
Begin:
	...
	`LOG("Perks for" @ AbilityContext.InputContext.AbilityTemplateName @ Perks.Length,, 'JediClassWOTCV2');
	`LOG("Unit.CurrentPerkAction" @ Unit.CurrentPerkAction,, 'JediClassWOTCV2');
