...
static simulated function XComGameState LeapStrike_BuildGameState_UNUSED(XComGameStateContext Context)
{
	...
	`LOG(GetFuncName() @ LastElementIndex,, 'JediClassWOTC');
	if (LastElementIndex > 0)
	{
		...
		`LOG(GetFuncName() @ NewTileLocation.X @ NewTileLocation.Y @ NewTileLocation.Z,, 'JediClassWOTC');
	}
...
static simulated function PrecomputedPathMovement_ModifyActivatedAbilityContext(XComGameStateContext Context)
{
	...
	if (EndTileLocation != EmptyTile)
	{
		...
		`LOG(GetFuncName() @ EndTileLocation.X @ EndTileLocation.Y @ EndTileLocation.Z,, 'JediClassWOTC');
	}
	...
	if (AbilityContext != none && AbilityContext.InterruptionStatus != eInterruptionStatus_Interrupt)
	{
		foreach AbilityContext.InputContext.MultiTargets(TargetRef)
		{
			...
			if (TargetUnit != none)
			{
				...
				if (DamageDealt > 0)
				{
					//`LOG("TargetUnit" @ TargetUnit.GetMyTemplateName() @ DamageDealt,, 'JediClassWOTC');
		...
		if (StolenHP > 0)
		{
			...
			if (NewSourceUnit != none)
			{
				...
				//`LOG("ForceDrainListener AbilityTriggerAgainstSingleTarget" @ AbilityState.ObjectID,, 'JediClassWOTC');
...
simulated function ForceChoke_BuildVisualization(XComGameState VisualizeGameState/*, out array<VisualizationTrack> OutVisualizationTracks*/)
{
	...
	if(Context.IsResultContextHit())
	{
		...
		//`LOG(AbilityTemplate.AbilityTargetEffects[0].Name @ Context.FindTargetEffectApplyResult(AbilityTemplate.AbilityTargetEffects[0]),, 'JediClassWOTC');
