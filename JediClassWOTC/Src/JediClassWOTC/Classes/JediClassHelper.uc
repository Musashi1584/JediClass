...
static function AddDarkSidePointToGameState(XComGameState_Unit Unit, out XComGameState NewGameState, int DarkSidePointsToAdd = 1)
{
	...
	if (NewSourceUnit != none)
	{
		...
		`LOG("JediClassHelper AddDarkSidePointToGameState for" @ NewSourceUnit.GetFullName() @ DarkSidePointsToAdd @ "(" @ DarkSidePoints.fValue + DarkSidePointsToAdd @ ")",, 'JediClassWOTC');
	}
}
...
static function AddDarkSidePoint(XComGameState_Unit Unit, int DarkSidePointsToAdd = 1)
{
	...
	if (NewSourceUnit != none)
	{
		...
		`LOG("JediClassHelper AddDarkSidePoints for" @ NewSourceUnit.GetFullName() @ DarkSidePointsToAdd @ "(" @ DarkSidePoints.fValue + DarkSidePointsToAdd @ ")",, 'JediClassWOTC');
	}
}
...
static function AddLightSidePointToGameState(XComGameState_Unit Unit, out XComGameState NewGameState, int LightSidePointsToAdd = 1)
{
	...
	if (NewSourceUnit != none)
	{
		...
		`LOG("JediClassHelper AddLightSidePointToGameState for" @ NewSourceUnit.GetFullName() @ LightSidePointsToAdd @ "(" @ LightSidePoints.fValue + LightSidePointsToAdd @ ")",, 'JediClassWOTC');
	}
}
...
static function AddLightSidePoint(XComGameState_Unit Unit, int LightSidePointsToAdd = 1)
{
	...
	if (NewSourceUnit != none)
	{
		...
		NewSourceUnit.SetUnitFloatValue('LightSidePoints', LightSidePoints.fValue + LightSidePointsToAdd, eCleanup_Never);
		`TACTICALRULES.SubmitGameState(NewGameState);
		`LOG("JediClassHelper AddLightSidePoints for" @ NewSourceUnit.GetFullName() @ LightSidePointsToAdd @ "(" @ LightSidePoints.fValue + LightSidePointsToAdd @ ")",, 'JediClassWOTC');
	}
}
