...
function UpdatePathingPawnTile(TTile TargetTile)
{
	...
	for (Index = PathingPawn.PossibleTiles.Length - 1; Index >=0; Index--)
	{
		...
		{
			`LOG(default.class @ GetFuncName() @ "Removing Tile" @ Index @ "from PossibleTiles" @ Tile.X @ Tile.Y @ Tile.Z,, 'JediClassWOTC');
...
function name ValidateTargetLocations(const array<Vector> TargetLocations)
{
	...
	if (TargetLocations.Length == 1)
	{
		...
		//`LOG(default.class @ GetFuncName() @ GrenadePathTile.X @ GrenadePathTile.Y @ GrenadePathTile.Z @ "/" @ PathTile.X @ PathTile.Y @ PathTile.Z,, 'JediClassWOTC');

		if (PathTile == GrenadePathTile)
		{
			//`LOG(default.class @ GetFuncName() @ "AA_Success",, 'JediClassWOTC');
			return 'AA_Success';
		}
	}

	//`LOG(default.class @ GetFuncName() @ "AA_NoTargets",, 'JediClassWOTC');
