...
function Init()
{
	...
	foreach FoundFireWeaponNotifyEvents(NotifyEvent)
	{
		...
		`LOG(default.class @ GetFuncName() @ `ShowVar(NotifyEvent.Time) @ `ShowVar(Notify),, 'JediClassWOTC');
		break;
	}
...
{	
	`LOG(default.class @ GetFuncName() @ `ShowVar(bMainImpactNotify) @ `ShowVar(HitLocation),, 'JediClassWOTC');
}
