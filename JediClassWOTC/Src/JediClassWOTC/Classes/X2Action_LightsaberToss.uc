...
{
	`log(default.Class @ GetFuncName() @ "AddProjectileVolley",, 'JediClassWOTC');
...
function Init()
{
	...
	`log(default.Class @ GetFuncName() @ LastLocation.X @ LastLocation.Y @ LastLocation.Z,, 'JediClassWOTC');
...
function SetProjectileColor()
{
	...
	if (MeshComp != none)
	{
		for (i = 0; i < MeshComp.GetNumElements(); i++)
		{
			...
			//`log(default.class @ GetFuncName() @ "MIC" @ i @ MIC @ MIC.Parent,, 'JediClassWOTC');
			if (InStr(MIC.Parent, "MAT_Lightsaber_Blade") != INDEX_NONE)
			{
				//`log(default.class @ GetFuncName() @ "MIC" @ "applying",, 'JediClassWOTC');
			...
			//`log(default.class @ GetFuncName() @ "MITV" @ i @ MITV @ MITV.Parent,, 'JediClassWOTC');
			if (InStr(MITV.Parent, "MAT_Lightsaber_Blade") != INDEX_NONE)
			{
				//`log(default.class @ GetFuncName() @ "MITV" @ "applying",, 'JediClassWOTC');
...
function SendProjectile(vector Source, vector Target, bool bReturnToSource = false)
{
	...
	`log("X2Action_LightsaberToss SendProjectile to" @ Target,, 'JediClassWOTC');
}

function NotifyTargetsAbilityApplied()
{
	`log("X2Action_LightsaberToss" @ GetFuncName() @ "was called",, 'JediClassWOTC');
}
...
function ProjectileNotifyHit(bool bMainImpactNotify, Vector HitLocation)
{
	...
	`log("X2Action_LightsaberToss ProjectileNotifyHit" @ bMainImpactNotify @ iProjectileNotifyHitIndex @ "/" @ Targets.Length @ HitLocation @ UnitPawn.Location,, 'JediClassWOTC');
	...
	if (bReturn)
	{
		...
		`log("X2Action_LightsaberToss bMainImpactNotify" @ bMainImpactNotify @ "/" @ iProjectileNotifyHitIndex @ "/" @ Targets.Length,, 'JediClassWOTC');
		`log("X2Action_LightsaberToss HitLocation" @ HitLocation,, 'JediClassWOTC');
		`log("X2Action_LightsaberToss RightHandLocation" @ RightHandLocation,, 'JediClassWOTC');
		...
		for (HandProximity = 0.0; HandProximity <= 100.0;  HandProximity+=0.1)
		{
			...
			{
				`log("X2Action_LightsaberToss AreVectorsDifferent" @ HandProximity @ class'Helpers'.static.AreVectorsDifferent(HitLocation, RightHandLocation, HandProximity),, 'JediClassWOTC');
				`log("X2Action_LightsaberToss returned to source" @ iProjectileNotifyHitIndex @ Targets.Length,, 'JediClassWOTC');
...
simulated state Executing
{
	...
	{
		//`log(bSnap,, 'JediClassWOTC');
		if (bSnap && !bCanComplete)
		{
			//`log("iNextNotifyIndex" @ iNextNotifyIndex @ "Targets.Length" @ Targets.Length,, 'JediClassWOTC');
			DoSnap();
	...
	simulated function DoSnap()
	{
		...
		`log(default.Class @ GetFuncName() @ iNextNotifyIndex @Targets.Length,, 'JediClassWOTC');

		if (iNextNotifyIndex >= Targets.Length)
		{
			UnitPawn.Mesh.GetSocketWorldLocationAndRotation('R_Hand', NextTargetLocation);
			`log("X2Action_LightsaberToss Return Lightsaber to location" @ NextTargetLocation,, 'JediClassWOTC');
		...
		else if (iNextNotifyIndex < Targets.Length)
		{
			...
			{
				`log("X2Action_LightsaberToss Shooting Lightsaber at target" @ NextTargetLocation @ Targets.Length @ iNextNotifyIndex,, 'JediClassWOTC');
...
Begin:
	...
	if (!bCanComplete)
	{
		`LOG("Start" @ IdleAnimationName,, 'JediClassWOTC');
	...
	if (bCanComplete && bReturnedToSource)
	{
		...
		`LOG(default.class @ "Start" @ StopAnimationName,, 'JediClassWOTC');
		LightsaberReturned();
		Params = default.Params;
