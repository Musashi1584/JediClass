...
)
{
	...
	foreach default.IgnoreEffects(CurrentEffect)
	{
		...
		{
			`LOG(default.class @ GetFuncName() @ "false, IgnoreEffects" @ CurrentEffect,, ,, 'JediClassWOTC');
			return false;
		}
	}
	...
	{
		`LOG(default.class @ GetFuncName() @ "false, is in overwatch",, ,, 'JediClassWOTC');
		return false;
	}
	...
	if (!AbilityState.IsMeleeAbility() && bIsPrimaryTarget)
	{
		...
		if (RandRoll < ModifiedHitChance)
		{
			...
			`LOG(default.class @ GetFuncName() @ NewHitResult @ bReflect,, 'JediClassWOTC');
			return true;
		}
	}
