...
{
	`LOG("ProjectileNotifyHit",, 'JediClassWOTC');
...
function OnProjectileHit(vector vLocation)
{
	...
	`LOG("OnProjectileHit",, 'JediClassWOTC');
	if (!ChainHasStarted())
	{
	...
	else
	{
		...
		`LOG("FindNearestTarget" @ i,, 'JediClassWOTC');
	}
	if (i != INDEX_NONE)
	{
...
function AdvanceChain(int iHitIdx, bool bDoNotifyTarget, vector vSourceLocation)
{
	...
	if (bDoNotifyTarget)
	{
		...
		`LOG(default.Class @ GetFuncName() @ "sent Visualizer_ProjectileHit to" @ TargetState,, 'JediClassWOTC');
	}
	for (i = 1; i < Targets.Length; i++)
	{
...
function SendProjectile(vector Source, vector Target)
{
	...
	`LOG("ProjectileTemplate" @ ProjectileTemplate,, 'JediClassWOTC');
...
Begin:
	...
	`LOG("Perks for" @ AbilityContext.InputContext.AbilityTemplateName @ Perks.Length,, 'JediClassWOTC');
	`LOG("Unit.CurrentPerkAction" @ Unit.CurrentPerkAction,, 'JediClassWOTC');
