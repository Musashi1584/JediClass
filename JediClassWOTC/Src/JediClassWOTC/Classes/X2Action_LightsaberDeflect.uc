...
function Init()
{
	...
	`LOG(default.class @ AbilityContext.ResultContext.HitResult @ bHit,, 'JediClassWOTC');
...
function SendProjectile(AnimNotify_FireWeaponVolley FireVolleyNotify)
{
	...
	//`log("X2Action_LightsaberDeflect DeflectTarget" @ SourceUnitState.ObjectID @ ReflectTargetID,, 'JediClassWOTC');
	...
	//`log("X2Action_LightsaberDeflect Target Unit" @ TargetUnitState.GetMyTemplateName() @ ReflectTargetID @ Target,, 'JediClassWOTC');
	if (bHit)
	{
	...
	else
	{
		...
		`log("X2Action_LightsaberDeflect Randomize Target" @ `ShowVar(Target),, 'JediClassWOTC');
		SpawnAndConfigureNewCosmeticProjectile(ProjectileTemplate, FireVolleyNotify, AbilityContext, Source, Target);
	}

	`log("X2Action_LightsaberDeflect SendProjectile" @ bHit @ ProjectileTemplate.ObjectArchetype,, 'JediClassWOTC');
}
...
function EventListenerReturn OnProjectileFromActionToWaitFor(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	if (!bSendProjectiles)
	{
		`log(default.Class @ GetFuncName() @ "sending projectiles",, 'JediClassWOTC');
...
{
	`log(default.Class @ GetFuncName() @ "stopping anim",, 'JediClassWOTC');
...
function CompleteAction()
{
	...
	`LOG("X2Action_LightsaberDeflect.CompleteAction",, 'JediClassWOTC');

	if (AbilityContext.ResultContext.EffectRedirects.Length > 0)
	{
		`LOG("X2Action_LightsaberDeflect Reseting bCanDamageNonFragile for " @ AbilityContext.InputContext.SourceObject.ObjectID,, 'JediClassWOTC');
...
simulated state Executing
{
	simulated event Tick(float fDeltaT)
	{
		...
		//`log(default.Class @ GetFuncName() @ "bSendProjectiles" @ bSendProjectiles,, 'JediClassWOTC');
		if (bSendProjectiles && !bProjectilesFired)
		{
			if (FireVolleyNotifies.Length == 0)
			{
				...
				`log("X2Action_LightsaberDeflect FireVolleyNotify" @ InstigatorUnitState.GetMyTemplateName() @ InstigatorUnitState @ FireVolleyNotifies.Length @ ProjectileTemplate,, 'JediClassWOTC');
			}
			...
			if (OutNotifyTimes.Length > 0 && CurrentVolleyIndex < OutNotifyTimes.Length)
			{
				...
				{
					`log("X2Action_LightsaberDeflect Sending Volley" @ CurrentVolleyIndex @ NextProjectileTime @ ProjectileTimer,, 'JediClassWOTC');
			...
			{
				`log("X2Action_LightsaberDeflect End anim",, 'JediClassWOTC');
				bProjectilesFired = true;
			}
			else if (DummyNotify != none)
			{
				`log("X2Action_LightsaberDeflect Send dummy projectile",, 'JediClassWOTC');
...
Begin:
	...
	if (UnitPawn.GetAnimTreeController().CanPlayAnimation(Params.AnimName))
	{
		...
		else
		{
			...
			{
				`log("X2Action_LightsaberDeflect PlayingSequence " @ bShouldContinueAnim,, 'JediClassWOTC');
			...
			//	FinishAnim(PlayingSequence);
			//}
			`log("X2Action_LightsaberDeflect FinishAnim",, 'JediClassWOTC');
			`LOG("*******************************************************************************************************",, 'JediClassWOTC');
		}
	}
	else
	{
		`log("X2Action_LightsaberDeflect Failed to play animation" @ Params.AnimName @ "on" @ UnitPawn @ "as part of" @ self,, 'JediClassWOTC');
	}
