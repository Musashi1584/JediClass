...
static function bool AbilityTagExpandHandler_CH(string InString, out string OutString, Object ParseObj, Object StrategyParseOb, XComGameState GameState)
{
	...
	`LOG(GetFuncName() @ InString @ "1" @ EffectState @ AbilityState @ AbilityTemplate @ ParseObj @ GameState.GetNumGameStateObjects(),, 'JediClassWOTC');
	...
	if (AbilityState != none)
	{
		...
		if (UnitState != none)
		{
			...
			`LOG(GetFuncName() @ InString $ ":" @ OutString,, 'JediClassWOTC');

			return OutString != "";
		}
	}

	`LOG(GetFuncName() @ InString @ "2" @ EffectState @ AbilityState @ AbilityTemplate,, 'JediClassWOTC');
...
static function UpdateWeaponMaterial(XGWeapon WeaponArchetype, MeshComponent MeshComp)
{
	...
	//`LOG(GetFuncName() @ XComWeapon(WeaponArchetype.m_kEntity) @ MeshComp.GetNumElements(),, 'JediClassWOTC');
	...
	if (MeshComp != none)
	{
		for (i = 0; i < MeshComp.GetNumElements(); ++i)
		{
			...
			//`LOG(GetFuncName() @ i @ XComWeapon(WeaponArchetype.m_kEntity) @ MaterialInstanceTimeVarying(Mat).Parent.Name @ MaterialInstanceConstant(Mat).Parent.Name,, 'JediClassWOTC');
			...
			if (MITV != none)
			{
				...
				//`LOG(GetFuncName() @ i @ MaterialInstanceTimeVarying(ParentMITV.Parent).Name @ MITV.Name,, 'JediClassWOTC');
				if (InStr(ParentMat, "MAT_Lightsaber_Blade") != INDEX_NONE)
				{
					...
					//	`LOG(GetFuncName() @ "setting blade mitv" @ MITV,, 'JediClassWOTC');
					//
					//	break;
					//}
					...
					if (Palette != none)
					{
						if(WeaponArchetype.m_kAppearance.iWeaponTint != INDEX_NONE)
						{
							...
							`LOG(GetFuncName() @ "Setting Emissive Color" @ MITV @ ParentMat,, 'JediClassWOTC');
						}
					}
...
static function UpdateResearch()
{
	...
	foreach class'X2StrategyElement_LightsaberTech'.default.LIGHTSABER_PROJECT_NAMES(ResearchName)
	{
		...
		{
			`log(default.class @ GetFuncName() @ ResearchName @ "not found, creating...",, 'JediClassWOTC');
...
{
	//`LOG("ForceLightning Ability" @ class'X2AbilityTemplateManager'.static.GetAbilityTemplateManager().FindAbilityTemplate('ForceLightning'),, 'JediClassWOTC');
...
static function WeaponInitialized(XGWeapon WeaponArchetype, XComWeapon Weapon, optional XComGameState_Item ItemState=none)
{
	...
	if(UnitState != none && HasDualLightsaberEquipped(UnitState))
	{
		if (IsPrimaryMeleeWeaponTemplate(WeaponTemplate) || IsSecondaryMeleeWeaponTemplate(WeaponTemplate))
		{
			...
			`LOG(default.Class.Name @ GetFuncName() @ "Adding Lightsaber_CV.Anims.AS_Lightsaber_Pawn_Dual",, 'JediClassWOTC');
		}
	}
}
...
exec function DebugGiveJediUpgrades(optional int NumToGive = 1)
{
	...
	foreach AllUpgrades(ThisUpgrade)
	{
		...
		{
			`LOG("Adding to HQ" @ ThisUpgrade.DataName,, 'JediClassWOTC');
...
exec function GiveDarkSidePoint(int Amount = 1)
{
	...
	if (NewSourceUnit != none)
	{
		...
		`GAMERULES.SubmitGameState(NewGameState);
		`LOG("EXEC AddDarkSidePoints for" @ NewSourceUnit.GetFullName() @ Amount @ "(" @ DarkSidePoints.fValue + Amount @ ")",, 'JediClassWOTC');
	}
}
